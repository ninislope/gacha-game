...
export const LoadScene = observer(({store}: BaseProps) => {
    ...
    if (store.loadState === "loaded") {
        ...
        const tip = store.sceneStates.load.loadingTip;
        return (
            <div className={scene} style={{background: "#ffc"}}>
                {tip ?
                    <div>
                        <div onClick={() => store.sceneStates.load.refleshTip()} style={center({top: "50%", height: "70%", width: "20em"}, {position: "absolute", color: "#666", background: "#fff", fontSize: "1.1em", borderRadius: "0.5em"})}>
